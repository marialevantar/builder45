  ...
  <div class="login-box-body">
    ...
    <form id="signInForm" method="post">
      <div class="form-group has-feedback">
        <input type="text" class="form-control" placeholder="Email" name="UserEmail" id="username">
...
<script>
  ...
  $('#signInForm').submit(function(event){
      $('#errorMsg').html('');
      $.ajax({
        url:"<?php echo base_url(); ?>tailor/login",
        method:"POST",
        data:$('#signInForm').serialize(),
        success:function(response) {
          var userobj = jQuery.parseJSON(response);
          if(userobj.status == 'success'){
              window.location.replace("<?php echo base_url(); ?>tailor/dashboard");
          }
          else {
