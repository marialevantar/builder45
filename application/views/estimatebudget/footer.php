...
<script>
    ...
    $('#header_selection').on('change', function() {
  var cid=this.value;  
  la_cost = $("#property_selection").val() ;
 
  location.href = "<?= base_url() ?>estimattebudget/addproject/"+cid+"/"+la_cost+"/";
});
function laborcharge()
{
    la_cost = $("#Labor_cost").val() ;
    ma_cost = $("#material_cost").val() ;
    $('#total_cost').val(Number($('#Labor_cost').val()) + Number($('#material_cost').val()));
    $('#variance_cost').val(Number($('#Labor_cost').val()) + Number($('#material_cost').val())- Number($('#actual_cost').val()));
    $('#amount_due').val(Number($('#actual_cost').val()) - Number($('#currentpaid').val()));
